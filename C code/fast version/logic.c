...
void ECDSA_256_sign(unsigned char sig_input[64], const unsigned char hash_input[32]){
    ...
    const char *pk_x_str = "C14CD88E3676B4B5D79E6547ED90DA43F2381EC20CA251925B2D0A7A1A594743";
    const char *pk_y_str = "948AF7B0B5FD6AF4B97CCE4DF4F4848A536E6243EFC80ACC4922C0FD8900F1B7";
    ...
    mpz_t var_in_P[5];
    mpz_t var_in_N[3];
    for(int i = 0; i < 5; i++) mpz_init(var_in_P[i]);
    for(int i = 0; i < 3; i++) mpz_init(var_in_N[i]);
    mpz_set(var_in_P[0], hash_val);
    while(1){
        for(int i = 0; i < mat_size; i++) for (int j = 0; j < mat_size + 1; j++) mpz_set_ui(coeff[i][j], 0);
        for(int i = 2; i < 5; i++) mpz_set_ui(var_in_P[i], 0);
        for(int i = 0; i < 3; i++) mpz_set_ui(var_in_N[i], 0);
        ...
        for (int round = 0; round < 256; round++){
            ...
            if(round == 0) {
                CoeffCal(coeff, coeffs_p, var_in_P, 2, 4, 27, p, 0, 1, 62, 3, 5);
            }
            else if(round < 255) {
                mpz_set_ui(var_in_P[4], mpz_tstbit(hash_val, round));
                CoeffCal(coeff, coeffs_p, var_in_P, 5, 4, 130, p, 108 + 520 * (round - 1), 1, 62, 3, 20);
            }
            else if(round == 255){
                mpz_set_ui(var_in_P[4], mpz_tstbit(hash_val, round));
                CoeffCal(coeff, coeffs_p, var_in_P, 5, 3, 255, p, 132188, 1, 62, 4, 50);
            }
            GaussElim(coeff, p);
            for(int i = 0; i < 4; i++)
                mpz_set(var_in_P[i], coeff[i][mat_size]);
        }
        int I[3] = {0};
        for(int J = 0; J < 125; J++){
            ...
            CoeffCal(coeff, coeffs_n, var_in_N, 3, 2, 18, n, 0, 0, 62, 2, 4);
            GaussElim(coeff, n);
            if (ECDSA_256_verify(coeff[0][mat_size], coeff[1][mat_size], hash_val_const) == TRUE){
                #ifdef PRINT_GUESSES
                printf("(%d,%d,%d),\n", I[0],I[1],I[2]);
                #endif
                unsigned char sig[64];
                mpz_export(sig, NULL, 1, 32, 1, 0, coeff[0][mat_size]);
                mpz_export(sig + 32, NULL, 1, 32, 1, 0, coeff[1][mat_size]);
